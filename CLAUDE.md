...
## Recent Changes
- [2026-09-12] **相依漏洞 29 → 0**（`dcf5ef0`）——GitHub 報 135 條是**按相依路徑逐條計**，`npm audit` 的去重數字是 29（3 critical／15 high／8 moderate／3 low）。光 `next` 一個套件就佔 42 條 Dependabot 警示。兩個 critical 對本站是**真的可利用**不是理論值：①`@clerk/nextjs` 7.0.5 的「**middleware 路由保護繞過**」（影響 >=7.0.0 <7.2.1），而 `middleware.ts` 用的正是那個 pattern（`clerkMiddleware` + `createRouteMatcher` + `auth.protect()` 保護 `/dashboard(.*)`）→ 升 **7.9.2** ②`next` 16.1.7 背了 **25 個 advisory**，含 **Windows 主機上的未認證 RCE**、**Image Optimization API 未認證 RCE**（<16.3.3）、內部 Server Function 端點未認證洩漏、六種不同的 middleware/proxy 繞過 → 升 **16.3.5**（minor bump，順帶解掉 postcss 與 sharp）。其餘全是 transitive，`npm audit fix` 解決。**沒用 `--force`、沒有任何 major 版本移動**，只動三個直接相依。`next`／`eslint-config-next` 依本 repo 慣例**改回精確釘死**（跟 react/react-dom 一致），不留 `npm install` 寫進去的 `^`。驗證：`npm audit` = 0、`tsc --noEmit` 乾淨、`next build` 34 條路由全過（含 middleware、sign-in/up、兩個 webhook）。⚠️ **未驗**：Clerk 升級後的實際執行行為。API 在 7.x 內沒變且對 7.9.2 typecheck 通過，但**部署後應該登出狀態開一次 `/dashboard` 確認仍會擋**。Dependabot 的數字要等它重掃才會掉。
